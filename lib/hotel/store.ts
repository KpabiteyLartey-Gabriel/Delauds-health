...
export const STORAGE_KEY = "gh-hotel-state-v2"
export const SESSION_KEY = "gh-hotel-session-v1"

function seedUsersPublic(): HotelUserPublic[] {
  return [
    { id: "u-admin", email: "admin@hotel.gh", role: "admin", fullName: "System Admin" },
    {
      id: "u-reception",
...
export function isRoomFreeForDates(
  ...
  return isRoomFreeForOccupancy(occ, roomId, checkInDate, checkOutDate, ignoreBookingId)
}
...
export function cancelBooking(
  ...
  if (b.status === "checked_in") return { error: "Cannot cancel after check-in." }
  ...
  next = syncRoomStatuses(next)
  next = pushAudit(next, session, "booking_cancelled", `Booking ${bookingId} cancelled`)
  return { state: next }
}
...
export function checkIn(
  ...
  const b = state.bookings.find((x) => x.id === bookingId)
  if (!b) return { error: "Booking not found." }
  if (b.status !== "booked") return { error: "Only booked stays can be checked in." }
  const bookings = state.bookings.map((x) =>
...
export function checkOut(
  ...
  const b = state.bookings.find((x) => x.id === bookingId)
  if (!b) return { error: "Booking not found." }
  if (b.status !== "checked_in") return { error: "Guest must be checked in first." }
  const bookings = state.bookings.map((x) =>
...
export function addRoom(
  ...
  next = pushAudit(next, session, "room_added", `Room ${room.roomNumber} at GHS ${priceGhs}`)
